...
Запуск:
    cd backend
    python -m db.init_db                                      # данные из backend/data/
    python -m db.init_db --reset                              # сброс + перезагрузка
    python -m db.init_db --data-dir "путь/к/данным"           # произвольная папка с данными
    python -m db.init_db --reset --data-dir "путь/к/данным"   # сброс + кастомная папка

Пример загрузки данных Хакасии:
    python -m db.init_db --reset --data-dir "C:/Users/valer/OneDrive/Документы/project-map-social/algorithm&data/data/республика_хакасия"
"""
...
# --data-dir переопределяет путь к данным
_data_dir_arg = next((sys.argv[i + 1] for i, a in enumerate(sys.argv) if a == "--data-dir" and i + 1 < len(sys.argv)), None)
DATA_DIR = _data_dir_arg if _data_dir_arg else os.path.join(BACKEND_DIR, "data")
...
def _load_geojson(filename: str) -> dict | None:
...
def import_final_areas(conn: psycopg2.extensions.connection) -> None:
    ...
    if data is None:
        print(f"      Пропуск: файл final_areas.geojson не найден в {DATA_DIR}")
        return
...
def import_road_nodes(conn: psycopg2.extensions.connection) -> None:
    ...
    if data is None:
        print(f"      Пропуск: файл road_big_nodes.geojson не найден в {DATA_DIR}")
        return
